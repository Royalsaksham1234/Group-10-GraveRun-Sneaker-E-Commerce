...
public class AdminDashboard extends javax.swing.JFrame {
    ...
    public AdminDashboard() {
        ...
        //  Initialize DAOs (View layer responsibility)
        AdminProductDAO productDAO = new AdminProductDAOImpl();
        AdminUserDAO userDAO = new AdminUserDAOImpl();
        AdminOrderDAO orderDAO = new AdminOrderDAOImpl();
        
        // Initialize Controllers with full dependency injection
        this.productController = new AdminProductController(productDAO);
        this.userController = new AdminUserController(userDAO);
        this.orderController = new AdminOrderController(orderDAO);
        this.dashboardController = new AdminDashboardController(productDAO, userDAO, orderDAO);
        
        //  CRITICAL: Initialize components FIRST
        initComponents();
        
        //  CRITICAL: Properly set window properties for absolute layout
        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setTitle("Admin Dashboard - Grave Run");
        setResizable(false);
        
        // CRITICAL: Set exact size BEFORE pack() for absolute layout
        setSize(1280, 720);
        setPreferredSize(new java.awt.Dimension(1280, 720));
        
        // Initialize panels and data
        initializePanels();
        updateDashboardStats();
        setupActionListeners();
        
        //  CRITICAL: Center window on screen
        setLocationRelativeTo(null);
        
        System.out.println("✅ AdminDashboard initialized successfully");
        System.out.println("📐 Window size: 1280x720");
    }
     ...
     * View only displays data - Controller handles business logic
     */
    private void updateDashboardStats() {
        // ✅ Get data from controller (single method call)
        DashboardStats stats = dashboardController.getDashboardStats();
        
        // ✅ Update UI labels with formatted data from controller
        ordersNumberLabel.setText(dashboardController.formatNumber(stats.getTotalOrders()));
        usersNumberLabel.setText(dashboardController.formatNumber(stats.getTotalUsers()));
        productNumberLabel.setText(dashboardController.formatNumber(stats.getTotalProducts()));
        revenueNumberLabel.setText(dashboardController.formatRevenue(stats.getTotalRevenue()));
        
        System.out.println("📊 Dashboard stats updated: " + stats.toString());
    }
    ...
    private void logoutAction() {
        ...
        if (confirm == javax.swing.JOptionPane.YES_OPTION) {
            // ✅ Clear session
            util.SessionManager.logout();
            
            // ✅ Close admin dashboard
            this.dispose();
            
            // ✅ Open customer dashboard (back to guest mode)
